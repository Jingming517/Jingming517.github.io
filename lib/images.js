...
export class StereoProcessor{
    ...
    CalThreshold(pic){
        return Math.round(pic.Width/10);
    }
    ...
    disparity(leftPic, rightPic, startpos, maxdisp){
        ...
        var diff;
        var threshlod = CalThreshold(leftPic);
        for (let j=0; j<leftPic.Height; j++){
            for (let i=startpos; i<maxdisp; i++){
                
                diff = Math.abs(leftPic.DataRows[j][i].Gray - rightPic.DataRows[j][i].Gray);
                if(diff<threshlod){
                    var anothermatch = this.disparity(leftPic, rightPic, i, this.Calmaxdisp(leftPic, i));
                    if(CompareDiff(anothermatch, diff, i,j)) break;
                }
            }
