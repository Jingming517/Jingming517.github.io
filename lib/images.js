...
export class StereoProcessor{
    ...
    disparity(leftPic, rightPic, startpos, maxdisp){
        ...
        var disp = maxdisp;
        for (let i=0; i<leftPic.Height; i++){
            for (let j=startpos; j<maxdisp; j++){
                //for each pixel
                var diff = Math.abs(leftPic.DataRows[i][j].Gray - rightPic.DataRows[i][j].Gray);
                if(diff<IMAGE_SETTINGS.DISP_THRESHLOD&&Math.floor(disp/2)>1){
                    disp = disp/2;
                    var anothermatch = this.disparity(leftPic, rightPic, i, Math.floor(disp));
                    if(!anothermatch <= diff) break;
                }
            }
            if(i==maxdisp) rtn.DataRows[i].push(new Pixel(0,0,0,0));
    ...
    GetDepthMap(searchlen, tolerance,focus,base){
        ...
        for (let i=0; i<disparity.Width; i++){
            for (let j=0; j<disparity.Height; j++){
                ...
                var temp = Math.round(255/(disparity.DataRows[i][j].Gray+1));
                DepthPic.DataRows[i].push(new Pixel(temp, 0, 0, disparity.DataRows[i][j].A));
            }
        }
