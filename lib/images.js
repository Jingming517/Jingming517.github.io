...
export class StereoProcessor{
    ...
    disparity(leftPic, rightPic, startpos, maxdisp){
        ...
        for (let i=0; i<leftPic.Height; i++){
            for (let j=startpos; j<maxdisp; j++){
                //for each pixel
                var diff = Math.abs(left.DataRows[i][j].Gray - right.DataRows[i][j].Gray);
                if(diff<IMAGE_SETTINGS.DISP_THRESHLOD){
                    var anothermatch = disparity(leftPic, rightPic, i, (maxdisp/2));
    ...
    GetDisparityMap(searchlen, tolerance){
		...
		var dispPic = this.disparity(leftPic, rightPic, 0, maxdisp);
        return dispPic;
    ...
    GetDepthMap(searchlen, tolerance,focus,base){
        ...
        for (let i=0; i<disparity.Width; i++){
            for (let j=0; j<disparity.Height; j++){
                ...
                DepthPic.DataRows[i].push(new Pixel(Math.round(255/(disparity.DataRows[i][j].R+1)), 0, 0, disparity.DataRows.A));
            }
        }
