...
export class Picture{
    ...
    */
    LoadImgData(imgdata, sourcewidth, sourceheight){
        ...
        for (let j=0; j<this.Height; j++){
            for (let i = 0; i < this.Width; i++) {
                ...
                this.DataRows[j].push(new Pixel(imgdata[_i],imgdata[_i+1],imgdata[_i+2],imgdata[_i+3]))
            }
        }
    }
    Getpixel(x,y){
    ...
    PicGrayMatrix(i, j){
        var matrix = [[this.DataRows[i-1||i][j-1||j].Gray, this.DataRows[i-1][j].Gray0, this.DataRows[i-1||i][j+1||j].Gray],
                      [this.DataRows[i][j-1||j].Gray,  this.DataRows[i][j].Gray,  this.DataRows[i][j+1||j].Gray],
                      [this.DataRows[i+1||i][j-1||j].Gray, this.DataRows[i+1||i][j].Gray, this.DataRows[i+1||i][j+1||j].Gray]];
        return matrix;
...
export class StereoProcessor{
    ...
    PreProcessing(pic, searchlen){ 
        ...
        for (let m=0; m<100; m++){
            ...
            for (let n=0; n<100; n++){
                picDown.DataRows[m][n] = pic.DataRows[Math.round(m*pic.Height/100)][Math.round(n*pic.Width/100)];
            }
        ...
        for(let i=0; i<(left.Width); i++){
            for (let j=0; j<left.Height; j++){
                ...
                if (PixDisp > 125){ //cannot find corresponding point
                    ...
                    DispPic.DataRows[i].push(new Pixel(Math.round(PixDisp*255/9), 0, 0, 255));
                }                
            }
        }
        return DispPic;
    ...
    GetDepthMap(searchlen, tolerance,focus,base){
        ...
        for (let i=0; i<disparity.Width; i++){
            for (let j=0; j<disparity.Height; j++){
                ...
                DepthPic.DataRows[i].push(new Pixel(Math.round(255/disparity.DataRows[i][j].R)||0, 0, 0, disparity.DataRows.A));
            }
        }
