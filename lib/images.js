    ...
    LoadImgData(imgdata, sourcewidth, sourceheight){
        ...
        for (let j=0; j<this.Height; j++){
            ...
            for (let i = 0; i < this.Width; i++) {
                ...
                row.push(new Pixel(imgdata[_i],imgdata[_i+1],imgdata[_i+2],imgdata[_i+3]));
            }
...
export class StereoProcessor{
    ...
    PreProcessing(pic, searchlen){ 
        ...
        for (let m=0; m<100; m++){
            ...
            for (let n=0; n<100; n++){
                picDown.DataRows[m][n] = pic.DataRows[Math.round(pic.Height/100)][Math.round(pic.Width/100)];
            }
        ...
        for (let i=0; i<picDown.Height; i++){
            ...
            for (let j=0; j<picDown.Width; j++){
                var blur = this.dotProduct(KERNAL_GAUSSIAN3x3, picDown.PicGrayMatrix(i, j), 3) || picDown.DataRows[i][j];
                row.push(new Pixel(blur, blur, blur, 255));
            }
            BlurPic.DataRows.push(row);
        }
        //Edge detection
        /*
        ...
        for(let i=0; i<(left.Width); i++){
            ...
            for (let j=0; j<left.Height; j++){
                ...
                if (PixDisp > 125){ //cannot find corresponding point
                    row.push(new Pixel(0, 0, 0, 0));
                }
                else {
                    row.push(new Pixel(Math.round(PixDisp*255/9), 0, 0, 255));
                }
                
            }
            DispPic.DataRows.push(row);
        }
        return DispPic;
    }
    ...
    GetDepthMap(searchlen, tolerance,focus,base){
        ...
        for (let i=0; i<disparity.Width; i++){
            ...
            for (let j=0; j<disparity.Height; j++){
                ...
                row.push(new Pixel(Math.round(255/disparity.DataRows[i][j].R)||0, 0, 0, disparity.DataRows.A));
            }
            DepthPic.DataRows.push(row);
        }
