...
export class StereoProcessor{
    ...
    PreProcessing(pic, searchlen){ 
        ...
        for (let m=0; m<IMAGE_SETTINGS.OUTPUT_IMG_HEIGHT; m++){
            for (let n=0; n<IMAGE_SETTINGS.OUTPUT_IMG_WIDTH; n++){
                var a = Math.floor(m*(pic.Height-1)/IMAGE_SETTINGS.OUTPUT_IMG_HEIGHT) || (pic.Height-1);
                var b = Math.floor(n*(pic.Width-1)/IMAGE_SETTINGS.OUTPUT_IMG_WIDTH) || (pic.Width-1);
                picDown.DataRows[m][n] = pic.DataRows[b][a];
            }
        }
        //Gaussian blur
    ...
    CalThreshold(pic){ //calculate maximum pixel difference
        return Math.round(pic.Width/400);
    }
        ...
        for (let j=0; j<IMAGE_SETTINGS.OUTPUT_IMG_HEIGHT;j++){
            for (let i=0; i<IMAGE_SETTINGS.OUTPUT_IMG_WIDTH;i++){ 
                ...
                if(count == threshlod){ //can't find corresponding point
                    ...
                    rtn.DataRows[j].push(new Pixel(255, 0, 0 ,255)); 
