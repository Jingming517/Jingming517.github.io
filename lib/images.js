...
export class Picture{
    ...
    PicGrayMatrix(i, j){
        ...
        if (i>0 && i<this.Height-1 && j>0 && j<this.Width-1) {
            matrix = [[this.DataRows[i-1][j-1].Gray, this.DataRows[i-1][j].Gray0, this.DataRows[i-1][j+1].Gray],
...
export class StereoProcessor{
        ...
        for (let i=0; i<pic.Height; i++){
            for (let j=0; j<pic.Width; j++){
                ...
                if(i==0||i==pic.Width-1||j==0||j==pic.Width-1){
                    ...
                    var blur = this.dotProduct(IMAGE_SETTINGS.KERNAL_GAUSSIAN3x3, pic.PicGrayMatrix(i, j), 3); // || pic.DataRows[i][j].Gray;
    ...
    Calmaxdisp(leftPic, i){  //maximum number of pixels search left
        var maxdisp;
        if (leftPic.Width-i > 80){
            var maxdisp = Math.round((leftPic.Width - i)/80);
        }
        else maxdisp = 0;
        return maxdisp;
        //return 5; //testing purpose
    }
    CalThreshold(pic){ //calculate maximum pixel difference
        return Math.round(pic.Width/200);
    }
    ...
    disparity(leftPic, rightPic, startpos, maxdisp){
        ...
        for (let j=0; j<leftPic.Height; j++){
            for (i=startpos; i<startpos+maxdisp; i++){
                ...
                if(diff<threshlod && disp>0){
                    var anothermatch = this.disparity(leftPic, rightPic, i, disp);  //recursion
                    if(this.CompareDiff(anothermatch, diff, i,j)) break;
                }
            }
            if(i==maxdisp) rtn.DataRows[i].push(new Pixel(0,50,0,255));
            else rtn.DataRows[i].push(new Pixel(50, 0, 0 ,255)); //diff
        }0
        return rtn;
    }
    */
    ...
    disparity(leftPic, rightPic, startpos, maxdisp){
        ...
        for (let j=0; j<IMAGE_SETTINGS.OUTPUT_IMG_HEIGHT;j++){
            for (let i=0; i<IMAGE_SETTINGS.OUTPUT_IMG_WIDTH;i++){ 
                //for each pixel
                var diff = threshlod;
                for(count=0; count<=maxdisp; count++){ 
                    ...
                    if(diff<threshlod) break;
                }
                if(count == threshlod){ //can't find corresponding point
                    rtn.DataRows[j].push(new Pixel(0,255,0,0));
                }
                else if(count == 0) rtn.DataRows[j].push(new Pixel(0,0,255,255));  //blue
                else {
                    rtn.DataRows[j].push(new Pixel(diff, 0, 0 ,255)); //red (most pixels here)
                }
            }
        }
        return rtn;
    ...
    GetDepthMap(searchlen, tolerance,focus,base){
        ...
        for (let i=0; i<disparity.Height; i++){
            for (let j=0; j<disparity.Width; j++){
                //var temp = Math.round(255/(disparity.DataRows[i][j]+1));
                var temp = disparity.DataRows[i][j];
                var g = disparity.DataRows[i][j].G;
                var b = disparity.DataRows[i][j].B;
                var a = disparity

                DepthPic.DataRows[i].push(new Pixel(temp, g, b, a));
            }
        }
